  ...
  Widget build(BuildContext context) {
    ...
    onLikedByMe(){
      _postDetailX.updateDetailBoardLike(
        ...
        postLikeCnt : _postDetailX.postDetail.value.isLikedByMe ? _postDetailX.postDetail.value.postLikeCnt - 1  : _postDetailX.postDetail.value.postLikeCnt + 1,
          ...
          Stack(
            children: [
                      ...
                      itemBuilder: (BuildContext context,int index) {
                        ...
                        if(index == 0 ){
                          ...
                          return  Container(
                            ...
                            child: Column(
                              children: [
                                ...
                                /// 게시판 상세 페이지 이미지리스트
                                Obx(()=>Container(
                                  height: _postDetailX.postDetail.value.postImages.isEmpty ? 0 : imageBoxSize,
                                  padding: EdgeInsets.only(left: contentsPadding.w),
                                  child: PostDifferentList(
                                    widgetList: [
                                      for (var i = 0; i < _postDetailX.postDetail.value.postImages.length; i++)
                                        Row(
                                          children: [
                                            Container(
                                              width: _postDetailX.postDetail.value.postImages.isEmpty ? 0 : imageBoxSize,
                                              height: _postDetailX.postDetail.value.postImages.isEmpty ? 0 : imageBoxSize,
                                              decoration: BoxDecoration(
                                                  borderRadius: BorderRadius.circular(20),
                                                  image: DecorationImage(
                                                      image: NetworkImage(_postDetailX.postDetail.value.postImages[i]),
                                                      fit: BoxFit.cover)),
                                            ),
                                            SizedBox(
                                                width: i == _postDetailX.postDetail.value.postImages.length -1 ? contentsPadding.w : 6.w
                                            )
                                          ],
                                        )
                                      ...
                                      Row(
                                        ...
                                        children: [
                                          Obx(() =>DetailBtn(
                                            ...
                                            onTap: onLikedByMe,
                                            active: _postDetailX.postDetail.value.isLikedByMe,
                                          ) )
                                          ,
                                        ],
                                      ),
                                      Obx(()=> CommentSide(
                                        imgCnt: _postDetailX.postDetail.value.postImages.length,
                                        likeCnt: _postDetailX.postDetail.value.postLikeCnt,
                                        commentCnt: _postDetailX.postDetail.value.postCommentCnt,
                                      ))
                                    ],
                                  ),
                                ),
                          ...
                          return Column(
                            children: [
                              Obx(() => CommentBox(
                                  authorId: _postDetailX.boardComment[commentInt].authorId,
                                  ...
                                  isBlind:  _postDetailX.boardComment[commentInt].isBlind,
                                  commentId:  _postDetailX.boardComment[commentInt].commentId,
                                  commentCreatedAt:  _postDetailX.boardComment[commentInt].commentCreatedAt,
                                  commentLikeCnt:  _postDetailX.boardComment[commentInt].commentLikeCnt,
                                  commentDesc:  _postDetailX.boardComment[commentInt].commentDesc,
                                  isLikedByMe:  _postDetailX.boardComment[commentInt].isLikedByMe,
                                ...
                                children: [
                                  for (var i = 0; i <  _postDetailX.boardComment[commentInt].replies.length; i++)
                                    CommentBox(
                                      ...
                                      authorNickname:
                                      _postDetailX.boardComment[commentInt].replies[i].authorNickname,
                                      authorProfileImg:
                                      _postDetailX.boardComment[commentInt].replies[i].authorProfileImg,
                                      isBlind:  _postDetailX.boardComment[commentInt].replies[i].isBlind,
                                      commentId:  _postDetailX.boardComment[commentInt].replies[i].replyId,
                                      commentCreatedAt:
                                      _postDetailX.boardComment[commentInt].replies[i].replyCreatedAt,
                                      commentLikeCnt:  _postDetailX.boardComment[commentInt].replies[i].replyLikeCnt,
                                      commentDesc:  _postDetailX.boardComment[commentInt].replies[i].replyDesc,
              ...
              Positioned(
                  bottom: 0,
                  left: 0,
                  child: PostDetailTextarea(postId: _postDetailX.postDetail.value.postId,)
              ),
            ],
          ),
        ]));
...
class PostDetailTextarea extends StatelessWidget{
  ...
  final postId;
  PostDetailTextarea({Key? key, @required this.postId})
      : super(key: key);
  @override
  Widget build(BuildContext context){
    final PostDetailController _postDetailX = Get.put(PostDetailController());
...
class DetailBtn extends StatelessWidget {
  ...
  final bool active;
  const DetailBtn(
      {Key? key, required this.name, required this.svg, required this.onTap ,required this.active})
      : super(key: key);
